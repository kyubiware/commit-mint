...
│   │   ├── skip-checks.ts      # Skip-checks preference persistence to config
│   │   ├── checks.test.ts      # Checks service tests
│   │   ├── clipboard.ts        # Cross-platform clipboard (wl-copy --foreground, wl-copy, xclip, xsel, pbcopy)
...
│   ├── ui/
│   │   ├── toggle-select.ts             # Generic select prompt with extensible inline hotkey toggles (auto-accept via `a`, skip-checks via `c`)
│   │   ├── auto-accept-select.test.ts   # Toggle select tests (legacy adapter)
...
│   │   └── staging-menu.ts     # Staging menu (file list, status labels, multi-select fallback, auto-accept `a` + skip-checks `c` hotkey toggles via selectWithToggles)
│   └── utils/
...
**New skip-checks behavior:** `src/services/skip-checks.ts` — extend `parseSkipChecksValue`, `getSkipChecks`, `setSkipChecks`

**New toggle behavior (new hotkey in staging menu):** `src/ui/toggle-select.ts` — add to the `toggles` array in `showStagingMenu` with a new `ToggleOption`; add persistence service if needed (follow `auto-accept.ts` or `skip-checks.ts` pattern)
